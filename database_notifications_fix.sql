-- ========================================
-- HABESHAEQUB - NOTIFICATION SYSTEM FIX
-- This script will fix the notification table creation issue
-- ========================================
...
-- Recreate the notifications table with correct structure
...
-- Recreate notification read status tracking table
...
-- Insert sample notification for testing (this should work now!)
...
-- Create the stored procedure for auto-assignment
DELIMITER //
CREATE PROCEDURE CreateNotificationForMembers(
...
-- Create sample notification reads for existing members (if any)
INSERT IGNORE INTO notification_reads (notification_id, member_id, is_read)
SELECT n.id, m.id, 0
FROM notifications n
CROSS JOIN members m
WHERE m.is_active = 1 AND n.target_audience = 'all_members';

-- Update recipient counts
UPDATE notifications n
SET total_recipients = (
    SELECT COUNT(*) FROM notification_reads nr WHERE nr.notification_id = n.id
),
total_unread = (
    SELECT COUNT(*) FROM notification_reads nr WHERE nr.notification_id = n.id AND nr.is_read = 0
),
total_read = (
    SELECT COUNT(*) FROM notification_reads nr WHERE nr.notification_id = n.id AND nr.is_read = 1
);
