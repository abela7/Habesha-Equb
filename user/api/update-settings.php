...
try {
    ...
    switch ($action) {
        case 'notifications':
            ...
            // Build notification preferences string
            $preferences = [];
            if ($email_notifications) $preferences[] = 'email';
            if ($sms_notifications) $preferences[] = 'sms';
            if ($payment_reminders && count($preferences) > 0) {
                $preferences = ['both']; // If reminders enabled with others, use 'both'
            } elseif ($payment_reminders) {
                $preferences[] = 'email'; // Default to email for reminders only
            }
            
            $notification_string = implode(',', $preferences);
            ...
            $result = $stmt->execute([$notification_string, $user_id]);
